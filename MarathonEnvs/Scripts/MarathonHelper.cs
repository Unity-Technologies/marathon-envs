...
namespace MLAgents
{
    public static class MarathonHelper
    {
		...
		{
			if (FlipMarathonX)
	    		return new Vector3(rightHanded.x, rightHanded.z, rightHanded.y); // use if fliping marathon's X direction
		...
		static public Quaternion ParseQuaternion(string str)
		{
			...
			// var q = MarathonFlipYZ ? new Quaternion(-x,z,-y,w) : new Quaternion(x,y,z,w);
			//var q = MarathonFlipYZ ? new Quaternion(x,z,y,w) : new Quaternion(x,y,z,w);
			return q;
		}		
